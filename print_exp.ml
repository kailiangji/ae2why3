...
let print_pfun fmt pp_tys pp_ty =
  List.iter (fun t -> fprintf fmt "%a " print_ppure_type t)
    pp_tys;
  fprintf fmt ": %a" print_ppure_type pp_ty
...
let print_pp_infix fmt (g, l, lexp, op) =
  match op with
  | PPand -> fprintf fmt "/\\"
  | PPor -> fprintf fmt "\\/"
  ...
  | PPiff -> fprintf fmt "<->"
  | PPlt ->
     if type_lexpr lexp g l = Is_Int then fprintf fmt "<"
     else fprintf fmt "<."
  | PPle ->
     if type_lexpr lexp g l = Is_Int then fprintf fmt "<="
     else fprintf fmt "<=."
...
let rec test_local_types g lib expr ty_lst =
  ...
  | PPinfix (e1, op, e2)->
     begin
       ...
       test_local_types g lib e2 ty_lst
     end
  | PPprefix (op, e1) ->
     test_local_types g lib e1 ty_lst
  | PPget (e1, e2) ->
  ...
  | PPset (e1, e2, e3) ->
     begin
       test_local_types g lib e1 ty_lst;
       test_local_types g lib e2 ty_lst;
       test_local_types g lib e3 ty_lst
     end
  | PPdot (e1, id) ->
     test_local_types g lib e1 ty_lst
  | PPrecord lbls -> ()
  ...
  | PPnamed (id, e1) -> test_local_types g lib e1 ty_lst
  | PPlet (id, e1, e2) ->
     begin
       test_local_types g lib e1 ty_lst;
       test_local_types g lib e2 ty_lst
     end
  | PPcheck e1 -> test_local_types g lib e1 ty_lst
  | PPcut e1 -> test_local_types g lib e1 ty_lst
  | PPcast (e1, pp_ty) ->
     begin
       test_types1 lib [pp_ty] ty_lst;
       test_local_types g lib e1 ty_lst
