...
let rec print_ppure_type chan_out ty =
  ...
  | PPTvarid (id,_) -> fprintf chan_out "'%s" (String.uncapitalize id)
...
let print_type chan_out (g, lib, ty) =
  ...
  | TypeDecl (_,vars, id, Enum id_lst) ->
     begin
       ...
       List.iter (fun id -> fprintf chan_out " | %s" id) (List.tl id_lst);
     end
  | TypeDecl (_,vars, id, Record lbl_lst) ->
     begin
       let lib_lst = ref [] in
       test_types2 lib lbl_lst lib_lst;
       List.iter (fun lib -> print_modules chan_out lib) (List.rev !lib_lst);
       fprintf chan_out "\n\ntype %s " id;
       List.iter (fun var -> fprintf chan_out "'%s " var) vars;
...
let print_fun_name chan_out id =
  let (h,t)= id in
  if String.length t = 0 then fprintf chan_out "%s " (String.uncapitalize h)
...
let rec print_lexpr chan_out (g, l, expr) =
  ...
  | PPapp (id, exprs) ->
     begin
	  ...
	  begin
       ...
       | "real_of_int" -> fprintf chan_out "(from_int%a)" print_lexprs (g, l, exprs)
  ...
  | PPconst const -> 
    begin
      ...
      | ConstReal n -> fprintf chan_out "%s" (string_of_float (Num.float_of_num n)) 
...
and print_triggers chan_out (g, l, exp_lst_lst) =
  ...
  | h :: tl ->
     begin
       ...
       let rec print_trigs chan_out trs =
	 match trs with
	 | [] -> fprintf chan_out "]"
	 | h1::tl1 ->
	    fprintf chan_out " | %a%a" print_trigger (g, l, h1) print_trigs tl1
       in print_trigs chan_out tl
     end
...
let print_pred_name chan_out id =
  let (n1, n2) = id in
  if String.length n2 = 0 then fprintf chan_out "%s " (String.uncapitalize n1)
  else fprintf chan_out "%s %s " (String.uncapitalize n1) n2
